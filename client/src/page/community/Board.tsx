...
const Board: FC = () => {
  ...
  const boardData = useSelector<{ board: { data: BoardData } }, BoardData>(
    (state) => state.board.data,
  );
  const boardStatus = useSelector<{ board: { status: string } }, string>(
    (state) => state.board.status,
  );
  const boardError = useSelector<{ board: { error: string } }, string>(
